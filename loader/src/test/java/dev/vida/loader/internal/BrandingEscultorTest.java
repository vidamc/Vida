...
final class BrandingEscultorTest {
    ...
    void does_not_patch_log_message_with_no_slash() {
        // Начинается как «Minecraft %s (» — but единственный %s внутри скобок,
        // не подходит под pattern (нужно >= 2).
    ...
    /**
     * Собирает класс, в котором метод {@code getLine(String...)} использует
     * {@code invokedynamic makeConcatWithConstants} с заданным шаблоном —
     * имитируем, как javac 21 компилирует DebugHud в MC 1.21+.
     */
    private static byte[] buildClassWithIndyConcat(
        ...
        // Дескриптор метода: (String × argCount) → String
    ...
    void might_match_on_indy_concat_template() {
        // Шаблон как у MC 1.21.1: 3 placeholder'а
    ...
    void patched_indy_class_is_jvm_loadable_and_returns_vida_brand() throws Exception {
        ...
        // Оригинальный метод принимает 3 строки; после патча лишние два arg
        // сбрасываются через POP, invokedynamic получает только первый.
    ...
    void is_f3_concat_template_recognises_variants() {
        ...
        assertThat(BrandingEscultor.isF3ConcatTemplate(
                "Minecraft \u0001 (\u0001/\u0001, \u0001)")).isTrue();
        assertThat(BrandingEscultor.isF3ConcatTemplate(
                "Minecraft \u0001 (loading)")).isTrue(); // 1 placeholder — пройдёт фильтр
        assertThat(BrandingEscultor.isF3ConcatTemplate(
                "Loading \u0001 mods")).isFalse();        // не начинается с нужного
        assertThat(BrandingEscultor.isF3ConcatTemplate(null)).isFalse();
    }

    /**
     * Чтобы поймать регрессы в сохранении stack-frame'ов после патча:
     * собираем целевой класс с ветвлениями (JVM обязывает в таких классах
     * иметь валидные frames), патчим его и реально загружаем+инвокируем.
     */
    @Test
    void patched_class_is_jvm_loadable_with_branches() throws Exception {
        ...
        // public String format(boolean flag, String ver) — ветвление требует frames.
