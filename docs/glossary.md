...
Страница отсортирована по алфавиту. Для каждого термина приводится краткое определение, откуда оно берётся в коде и где читать дальше.
...
_«эскультор брендинга»_. Внутренний `Escultor`, переписывающий format-строку F3-дебаг-экрана Minecraft — убирает `vida-<ver>/vanilla` и оставляет аккуратное `Minecraft 1.21.1 (vida)`.

- Класс: `dev.vida.loader.internal.BrandingEscultor`
...
_исп. «скульпторы»_. Низкоуровневые трансформеры классов. В отличие от `Vifada` (декларативной), `Escultor` работает напрямую с `ClassNode` из ASM. Предназначен для внутренних задач загрузчика и мод-инженерных сценариев.

- Документация: [guides/escultores.md](./guides/escultores.md)

### Fuente

_исп. «источник»_. Запланированный API для ресурсов и данных (текстуры, модели, языковые файлы, рецепты, структуры). Ещё не реализован — см. [roadmap](./roadmap.md).
...
_исп. «биение сердца»_. Высокопроизводительная система событий. Диспетчеры генерируются через `LambdaMetafactory`, без рефлексии на hot-path. Приоритеты, отмена, fan-out в секунду для стандартных событий — в низкоаллокационном режиме.
