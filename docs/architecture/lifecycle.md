...
Внутри одной фазы жизненного цикла порядок модов определяется _топологической сортировкой_ графа зависимостей:
...
2. **Dev-режим hot-reload.** `vidaRun`-задача Gradle поддерживает точечный перезапуск мода без рестарта JVM. Это preview-функциональность, и для production-сборок она отключена.
