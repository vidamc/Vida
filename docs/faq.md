...
### Где посмотреть полный список страниц документации?

На [главной странице `docs/`](./index.md) — таблицы по разделам (архитектура, все модули, guides, reference, миграции, безопасность). Отдельно: [справочники](reference/index.md), [гайды](guides/index.md), [обзор модулей](modules/index.md).

### Где описан data-driven datapack (`vida:dataDriven`)?

Модуль [:fuente](modules/fuente.md), поле манифеста — в [reference/manifest-schema.md](reference/manifest-schema.md). Полный data-driven mod собирайте в своём дереве согласно `manifest-schema` (отдельный JAR c datapack under `{datapackRoot}`).

### Как проверяются примеры кода в документации?

Задача `./gradlew vidaDocTest` компилирует Java из fenced-блоков в `docs/**/*.md`, если указан полный пакет `package dev.vida....`. Подробности — в описании модуля `:vida-doc-test` и в [`CONTRIBUTING.md`](../CONTRIBUTING.md).
